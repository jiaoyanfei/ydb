...
router.get('/', function(req, res, next) {
	...
	if(com.isLogined(req.session) )
	{
		...
		{
			selectSQL += " order by ProductId asc , OrderAmount asc";
			flagOrder ++;
		}
		else if(reqOrderAmount != undefined && reqOrderAmount == "定量down")
		{
			selectSQL += " order by ProductId asc , OrderAmount desc";
			flagOrder ++;
		}
		if(reqOrderNumber != undefined && reqOrderNumber == "编号up")
		{
			...
			{
				selectSQL += " order by ProductId asc , OrderNumber asc";
			}
		...
		else if(reqOrderNumber != undefined && reqOrderNumber == "编号down")
		{
			...
			{
				selectSQL += " order by ProductId asc , OrderNumber desc";
			}
		...
		com.executeSQL(selectSQL, function(err, rows) {
		    ...
		    else
		    {
		    	...
		    	for(var i = 0;i<rows.length;i++)
		    	{
		    		...
		    		if(i > 0 && rows[i]['ProductId'] != rows[i-1]['ProductId'])
		    		{
		    			myRows.push(rows[i]);
