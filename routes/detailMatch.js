...
router.get('/', function(req, res, next) {
	req.session.curPage = "detailMatch";
	...
	if(com.isLogined(req.session) )
	{
		...
		var selectSQL2 ='select MatchProductIDs from matches where Id = '+id;
		...
		com.executeSQL(selectSQL2,function(err,rows){
			...
			else
			{
				...
				var matchIds = rows[0]['MatchProductIDs'].split(',');
				req.session.MatchProductIDs = matchIds;
